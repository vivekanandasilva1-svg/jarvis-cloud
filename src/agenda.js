// agenda interna do app - funciona 100% sozinha (Postgres). Quando o Google Agenda estiver
// conectado (ver googleCalendar.js), cada operacao tambem espelha pra la - mas se o Google
// falhar por qualquer motivo, o evento local ainda fica salvo (a sincronizacao e "best effort",
// nunca trava a agenda por causa de uma falha externa).
...
export async function criarEvento({ titulo, descricao, local, inicio: inicioISO, fim: fimISO }) {
  ...
  if (await googleCalendar.estaConectado()) {
    try {
      googleEventId = await googleCalendar.criarEvento({ titulo, descricao, local, inicio, fim });
    } catch (err) {
  ...
  const { rows } = await pool.query(
    'INSERT INTO agenda_eventos (titulo, descricao, local, inicio, fim, google_event_id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
    [titulo, descricao || null, local || null, inicio, fim, googleEventId],
  );
  return { id: rows[0].id, sincronizadoComGoogle: !!googleEventId };
}

export async function listarEventos(fromISO, toISO) {
  if (!pool) return [];
  await tabelaPronta;
  ...
  const { rows } = await pool.query(
    'SELECT id, titulo, descricao, local, inicio, fim, google_event_id FROM agenda_eventos WHERE inicio >= $1 AND inicio <= $2 ORDER BY inicio ASC',
    [from, to],
  );
  const locais = rows.map((r) => ({ ...r, origem: 'lumia' }));

  if (!(await googleCalendar.estaConectado())) return locais;
  try {
    const googleEventos = await googleCalendar.listarEventos(from, to);
...
export async function cancelarEvento(id) {
  ...
  await tabelaPronta;
  const { rows } = await pool.query('SELECT google_event_id FROM agenda_eventos WHERE id = $1', [id]);
  if (!rows.length) throw new Error(`Nao achei nenhum evento com id ${id}.`);

  if (rows[0].google_event_id && (await googleCalendar.estaConectado())) {
    try {
      await googleCalendar.cancelarEvento(rows[0].google_event_id);
    } catch (err) {
